...
def affine_batch(X, rng, chunk=8192):
    ...
    for s0 in range(0, N, chunk):
        ...
        ang = rng.uniform(-8, 8, n).astype(np.float32) * np.pi / 180
        sc  = rng.uniform(0.92, 1.08, n).astype(np.float32)
        ty  = rng.uniform(-1.6, 1.6, n).astype(np.float32)
        tx  = rng.uniform(-1.6, 1.6, n).astype(np.float32)
        ident = rng.random(n) < 0.45
...
ps = init_params(rng)
_b = np.load("./best.npz"); ps = {k: _b[k].copy() for k in ["W1","b1","W2","b2","W3","b3"]}
...
np.savez("./final_ft.npz", **ps_best, test_acc=te)
