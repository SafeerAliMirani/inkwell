...
@vertex fn cv(@builtin(vertex_index) vi: u32, @location(0) src: vec3<f32>, @location(1) dst: vec3<f32>, @location(2) w: f32, @location(3) sidxf: f32) -> COut {
  ...
  let half = clamp(0.35 + strength * 2.0, 0.35, 2.8) * U.unit;
  ...
  var fade = 1.0;
  if (U.mode == 1u) { fade = clamp(2.8 / distance(U.camPos, mid), 0.22, 1.0); }
...
@fragment fn cf(o: COut) -> @location(0) vec4<f32> { return vec4<f32>(o.rgb, 1.0); }` });
  ...
  function render(state, pred) {
    ...
    uf[28] = state.front; uu[29] = mode; uu[30] = state.showAll ? 1 : 0; uu[31] = state.sign ? 1 : 0;
    ui[32] = pred; uf[33] = mode === 1 ? 0.008 : 1.0; uf[34] = canvas.width;
