...
**Ink a digit and watch the exact 3Blue1Brown network recognise it, live, with the matrix maths running in raw WGSL on your GPU.**
...
Draw a digit in the box and a real neural network classifies it as you draw. It is the small network from 3Blue1Brown's "But what is a neural network?", a 784-16-16-10 perceptron of about 13,000 parameters, drawn as flat columns of neurons joined by weighted connections. The matrix multiplies run in hand-written WGSL compute shaders, and the activations lighting the neurons are read back from those same buffers. No TensorFlow.js, no ONNX, no three.js, no d3. The network runs on the same GPU that draws it.
...
- **Hover any neuron.** A hidden-layer-1 neuron shows its 784 weights as a 28x28 picture, the pattern it looks for. Blue is a positive weight, red is negative.
...
4. `build/build_mvp.py` assembles the single `index.html` from `index_template.html` and the parts: `part_wgsl.js` (WGSL compute inference and the instanced neuron and connection renderer), `part_hover.js`, `part_sample.js`, and a Canvas2D fallback in `part_fallback.js`.
...
- The GPU is not needed for speed here, the network is tiny. Running it in WGSL is a deliberate choice, so the activations you see are the buffers the compute pass just wrote, with no library in between.
